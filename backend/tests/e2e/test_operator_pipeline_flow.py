...
async def test_operator_can_create_activate_and_inspect_pipeline(
    ...
    Steps:
    1. Create a 3-step pipeline (collect → process → export)
    2. Validate all step references resolve
    3. Activate the pipeline
    4. Verify activation record and pipeline status
    """
...
async def test_operator_dashboard_reflects_work_item_counts_and_completion(
    ...
    2. Create work items simulating file detection
    3. Process one work item through the pipeline
    4. Verify status shows correct counts and completion state
    """
    coll_inst, _ = e2e_instances["collector"]
    proc_inst, _ = e2e_instances["processor"]
    exp_inst, _ = e2e_instances["exporter"]
    ...
    status = await pipeline_manager.get_pipeline_status(pipeline.id)
    assert status.work_item_count == 3  # all 3 detected
    assert status.status == "ACTIVE"
...
async def test_pipeline_validation_blocks_invalid_activation(
    ...
    await pipeline_manager.add_step(
        ...
        step_type="COLLECT",
        ref_type="COLLECTOR",
        ref_id=uuid.uuid4(),  # does not exist
    )
